...
import { ArrowRight, Eye, Globe, Heart, Shield, Sparkles, Target } from "lucide-react";
...
function About() {
  return (
    <div className="animate-fade-in">
      ...
      <PageSection border className="bg-surface/40" id="leadership">
        <div className="mb-12 text-center md:mb-14">
          <p className="section-label">Our Leadership</p>
          <h2 className="mt-4 text-4xl md:text-5xl">
            The People Behind <span className="italic fg-text-gradient-gold">FG Media</span>
          </h2>
          <p className="mx-auto mt-5 max-w-3xl text-sm leading-relaxed text-muted-foreground md:text-base">
            FG Media&apos;s vision is shaped by leaders spanning technology, media, safety, and
            automotive innovation. Tap any profile to read their full story.
          </p>
        </div>
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {LEADERS.map((leader) => (
            <LeaderCard key={leader.id} leader={leader} variant="portrait" />
          ))}
        </div>
