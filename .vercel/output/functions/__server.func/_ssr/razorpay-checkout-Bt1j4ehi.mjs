import { d as postSponsorshipCreateOrder, f as postSponsorshipPayment, l as postNominationCreateOrder, u as postNominationPayment } from "./api-client-povPFUIH.mjs";
import { n as toast } from "../_libs/sonner.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/razorpay-checkout-Bt1j4ehi.js
...
async function openSponsorshipRazorpayCheckout(input, reservationId, onSuccess, onDismiss) {
	...
	if (order.isTestCharge) {
		...
		toast.message(`Checkout will charge ₹${testInr} for testing (advance shown: ₹${displayInr}).`);
	}
...
async function openNominationRazorpayCheckout(input, onSuccess, onDismiss) {
	...
	if (order.isTestCharge) {
		const testInr = (order.amount / 100).toLocaleString("en-IN");
		const displayInr = (order.displayAmountPaise / 100).toLocaleString("en-IN");
		toast.message(`Checkout will charge ₹${testInr} for testing (nomination fee shown: ₹${displayInr}).`);
	}
	return new Promise((resolve, reject) => {
		let settled = false;
		const finish = (fn) => {
